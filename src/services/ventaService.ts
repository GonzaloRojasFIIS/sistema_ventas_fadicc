...
export async function getVentasByVendedor(vendedorId: string, limit = 100): Promise<VentaComercial[]> {
  if (supabase) {
    const { data, error } = await supabase
      .from('ventas_comerciales')
      .select(`
  ...
  const ventas = getLocalData<VentaComercial[]>('fadicc_ventas', []);
  return ventas.filter(v => v.vendedor_id === vendedorId).slice(0, limit);
}
